...
export const getRoiSectionsVisibility = createSelector(
  getRoiCalculatorOutputsState,
  getSelectedEntity,
  (state: State, selectedEntity: RoiCalculatorOutputsEntity) => {
    return {
      showCurrentState: selectedEntity?.currentState?.earningCumulativeProb50
        ?.length
        ? true
        : false,
      showGoalState: selectedEntity?.roiCalculatorInputs?.goalStateOnetCode
        ?.length
        ? true
        : false,
      // TODO: update with alumni data
      showAlumniData: false,
      showLoanAccumulation: selectedEntity?.roiCalculatorInputs?.avgNetPrice
        ? true
        : false,
      showLoanPayoff: selectedEntity?.goalState?.outOfPocket50?.[0]
        ? true
        : false,
